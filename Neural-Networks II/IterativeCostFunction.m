...
costFunc = zeros(1,num_labels)
costFuncAllExamples = zeros(size(y,1),1)
for l = 1:size(y,1)
    for j = 1:num_labels
        costFunc(j) = (-yBinary(l,j) * log(hx'(l,j))) - ((1-yBinary(l,j))*log(1-hx'(l,j)))
        %kElemOfAllY =yBinary(:,j) 
        %costFuncVectorized = (-kElemOfAllY' * log(hx'(:,j))) - ((1-kElemOfAllY')*log(1-hx'(:,j)))
    end
    costFuncAllExamples(l) = sum(costFunc)
end

pause
J = 1/m * sum(costFuncAllExamples)
pause
disp('aqui J')
disp(J)
pause
