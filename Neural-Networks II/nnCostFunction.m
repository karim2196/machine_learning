...
% ------------------PART 2------------------
%here goes backpropagation algorithm
num_layers = 2 %(hidden, output) layer 1 is irrelevant for calculating deltas
deltaMayus1 = 0
deltaMayus2 = 0
for t = 1:m

    %STEP 1 - calculate the activations : z(2), a(2), z(3), a(3)
    a1 = X(t,:)' %1 row x 401 cols, we already added the bias unit before
    a1 = [1;a1]
    z2 = a1'*Theta1'
    %z2 is 1x25
    a2 = sigmoid(z2)
    ...
    z3 = a2*Theta2' %theta2 no transposed is 10x26
    a3 = sigmoid(z3)

    %STEP 2 - calulate deltas for every k output unit
    delta3 = a3 - yBinary(t,:)

    %STEP 3 - calculate delta of hidden layer (delta2)
    %OJO, LA PRIMERA COLUMNA DE THETA NO ES CALCULA EN BACKPROPAGATION
    %LLEGIR *STEP 4* : https://www.coursera.org/learn/machine-learning/discussions/all/threads/a8Kce_WxEeS16yIACyoj1Q
    delta2 = transpose(Theta2(:,2:end))*delta3' .* sigmoidGradient(z2)'

    %STEP 4 - accumulate the gradient
    %just compute for L = 2,3,...
    deltaMayus1 =  deltaMayus1 + ( delta2 * a1')
    deltaMayus2  = deltaMayus2 + ( delta3' * a2)

end

%STEP 5 - unregularized gradient
Theta1_grad = 1/m * deltaMayus1
Theta2_grad = 1/m * deltaMayus2

%Regularized gradient
%Theta1_grad(:,1) = 1/m * deltaMayus1(:,1)
